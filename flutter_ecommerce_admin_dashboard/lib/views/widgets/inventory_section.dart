...
import 'package:flutter_ecommerce_admin_dashboard/controllers/inventory_controller.dart';
...
class _InventorySectionState extends State<InventorySection> {
  ...
  Widget _buildDashboardCards() {
    return Row(
      ...
      children: [
        ...
        _BuildCard("Out of Stock", "150", Icons.cancel, Colors.redAccent),
        _BuildCard("Reorder Items", "10", Icons.reorder, Colors.blueAccent),
      ],
    );
  }
  ...
  DataRow _buildDataRow(Map<String, dynamic> item) {
    return DataRow(
      cells: [
        DataCell(Text(item['productId']?.toString() ?? "N/A")),
  ...
  List<Map<String, dynamic>> _applyFilers(List<Map<String, dynamic>> data) {
    ...
    return data.where((item) {
      if (searchText.isNotEmpty &&
          !item['productName'].toString().toLowerCase().contains(searchText))
        return false;
      return true;
    }).toList();
  }
